...
export const getOpenWeatherMapData = async (lat = 39.0321, lon = -77.4161) => {
  console.log(
    USE_FAKE_DATA === "true"
  ...
  const rawData =
    USE_FAKE_DATA === "true"
      ...
      : await (
          await fetch(
            `https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&exclude=minutely&appid=${OPEN_WEATHER_MAP_KEY}&units=imperial`,
          )
        ).json();
  const data = {
    current: {
    ...
    hourly: [
      ...rawData.hourly.slice(0, 12).map((h) => ({
        ...h,
        timezone: rawData.timezone,
        dt_offset: rawData.timezone_offset,
...
const doLogs = (data) => {
  ...
  console.log(
    `Today - Low: ${data.current.today.temp.min}, High: ${data.current.today.temp.max} | ${data.hourly.map((d) => `[${new Date(d.dt * 1000).getHours() + 1}:00: ${d.temp}]`).join(" ")}`,
  );
  console.log(
    `${data.daily.map((d) => `${dayNames[new Date(d.dt * 1000).getDay() ?? 0]}: ${d.temp.day}`).join(" ")}`,
  );
};
