...
export const TodaySection = ({ currentData, hourlyData }) => (
  <section style={sectionStyles}>
    <BigTempSection data={currentData} />
    <div style={nextTwelveHoursStyles}>
      <TempChart temps={hourlyData.map((d) => d.temp)} />
      {hourlyData.map((d, i) => (
        <HourItem data={d} />
      ))}
    </div>
...
const HourItem = ({ data }) => {
  ...
  return (
    <div style={hourStyles}>
      ...
      <h2 style={{ margin: 0, fontWeight: 800, fontSize: 32 }}>
        {data.temp.toFixed(0)}°
      </h2>
      <img
        style={{ width: "75%" }}
...
const BigTempSection = ({ data }) => {
  ...
  return (
    <div style={bigTempSectionStyles}>
