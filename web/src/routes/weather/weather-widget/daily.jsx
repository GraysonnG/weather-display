...
const getUVFillColor = (index) => {
  const colors = ["#5080b8", "#608050", "#f0e050", "#a02020", "#000000"];
...
export const DailySection = ({ data }) => {
  return (
    <div style={dailySectionStyle}>
      {data.map((dayData, i) => (
        <DayItem data={dayData} last={i === 6} />
      ))}
    </div>
  );
};
...
const DayItem = ({ data, last }) => {
  ...
  return (
    <div style={last ? lastItemStyle : dayItemStyle}>
      <div style={dayInfoSectionStyle}>
        <div
          style={{ display: "flex", marginTop: -7, flexDirection: "column" }}
        >
    ...
    <div style={moreSectionRowStyles}>
      <div style={moreSectionColStyles}>
        ...
        <span style={moreSectionItemValueStyles}>
          {mmToInches(data.rain ?? 0).toFixed(1)} in
        </span>
